...
#define OUTCODE(x,y) (((x)<xmin)?1:(((x)>xmax)?2:0))+(((y)<ymin)?4:(((y)>ymax)?8:0))
	...
	// This function is based around the code from the first Programming 101 lecture 
	void Surface::CopyFromTo(int src_position_x, int src_position_y, int src_pitch, Surface* dst, int dst_position_x, int dst_position_y)
	{
		Pixel* dst_buffer = dst->GetBuffer();
		Pixel* src_buffer = this->GetBuffer();

		int end_x = src_position_x + src_pitch;
		int read_x = src_position_x;
		int read_y = src_position_y;

		int dst_height = dst->GetHeight();
		int dst_pitch = dst->GetPitch();
		int dst_width = dst->GetPitch();

		for (int y = 0; y < dst_height; y++)
		{
			for (int x = 0; x < dst_width; x++)
			{
				dst_buffer[x + y * dst_pitch] = src_buffer[read_x + read_y * src_pitch];

				read_x++;
			}

			read_x = src_position_x;
